...
public class UserDupeIPTask implements Runnable {
    ...
    public void run() {
        if (!App.getDatabase().hasUserFlaggedLastIPAlert(user.getId())) {
            App.getLogger().debug(System.out.printf("User %d has not been flagged, running checks...%n", user.getId()));
            List<Integer> uids = App.getDatabase().getDupedUsers(channel.getSourceAddress().getHostName(), user.getId());
            if (uids != null && uids.size() > 0) {
                App.getLogger().debug(System.out.printf("User %d has %d accounts, iterating and sending report%n", user.getId(), uids.size()));
        ...
        } else App.getLogger().debug(System.out.printf("User %d has already been flagged. Skipping%n", user.getId()));
    }
}